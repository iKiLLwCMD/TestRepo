...
@Using Html.BeginForm(Nothing, Nothing, FormMethod.Post, New With {.enctype = "multipart/form-data"})
    ...
    @<fieldset>
        ...
        @Html.DropDownList("ListingTypeId", Nothing, New With {.style = "width:290px;padding: 10px 5px;", .class = "inputs", .placeholder = "Listing Type"})
        @Html.ValidationMessageFor(Function(model) model.ListingTypeId)
        <br />
       
        @Html.DropDownList("ListingPayId", Nothing, New With {.style = "width:290px;padding: 10px 5px;", .class = "inputs", .placeholder = "Listing Pay"})
            ...
            <tr>
                <td>
                    <div id="divLogo" class="inputs">
                        <div><img src="@Url.Content("~/UploadedListings/" & Model.Image1)"/></div><br />
                        <div class="inputWrapper" style="width: 100px;">Change Logo<input class="fileInput" id="Image1" type="file" name="Files" value="" onchange="addFileNamesToUploadControl(id, 'divLogo')" accept="image/*"/></div>
        ...
        <div id="divListingImages" class="inputs">
            @If Model.Image2.IsEmpty = False And Model.Image3.IsEmpty = False Then
                @<table>
                    <tr>
                        @If Model.Image2.IsEmpty = False Then
                            @<td>
                                <img style="width:120%;" src="@Url.Content("~/UploadedListings/" & Model.Image2)"/></td>        
                        End If
                        @If Model.Image3.IsEmpty = False Then
                            @<td>
                                <img style="width:120%;" src="@Url.Content("~/UploadedListings/" & Model.Image3)"/><br />
                            </td>
                        End If
                    </tr>
                </table>
            End If
            <div class="inputWrapper">Change Listing Images<input class="fileInput" id="fileImages" type="file" name="Files" value="" multiple="multiple" onchange="addFileNamesToUploadControl(id, 'divListingImages')" accept="image/*"/></div>
        </div>
        <br />
        <div class="editor-field">
        ...
        <br />
        <p>
            <input class="vcvbutton" type="submit" value="Edit" />
        </p>
    </fieldset>
            End Using
...
<script>
    ...
    $(function () {
        ...
        if (isChecked) {
            $("#EmailTemplateId").val(@Model.EmailTemplateId);
            $("#EmailTemplateId").change();
            $("#save").hide();
            $("#cancel").hide();
